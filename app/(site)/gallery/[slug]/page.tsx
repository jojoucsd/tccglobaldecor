...
// Awards: 4-item loop (include the new slug!)
const AWARD_SLUGS = [
...
/** Optional: human-friendly titles for award slugs not in GALLERY */
const AWARD_TITLES: Record<string, string> = {
  "sands-supplier-excellence-award": "Sands Supplier Excellence Service Award",
  "marina-bay-singapore-award": "Marina Bay Sands Recognition",
  "gold-key-award": "Grand Imperial Court Winner & Honoree",
  "thedesignawards": "The Design Awards Winner",
};
...
export function generateStaticParams() {
  // Build params from both GALLERY and the award loop
...
export default async function GalleryPage({
  ...
  const prettyTitle = entry?.title ?? AWARD_TITLES[slug] ?? titleFromSlug(slug);

  type Loop = readonly string[] | null;

  const inSpec = (SPECIALIZATION_SLUGS as readonly string[]).includes(slug);
  const inAwards = (AWARD_SLUGS as readonly string[]).includes(slug);
