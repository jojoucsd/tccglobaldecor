...
### RAG Knowledge Base (Planned — Phase 5) — goals re-scoped 2026-08-13, deadline: before BDNY '26 (Nov 8-9)

**Baseline corpus generated 2026-08-12:** `scripts/export-rag-corpus.mjs` exports the site's own English content (30 projects from Supabase + 9 company/capability topic files from `messages/en.json` and a few small static data files) into `/rag/projects/*.md` + `/rag/company/*.md`, browsable read-only via `/admin/rag`. This is the "company facts"/onboarding-wiki layer — real, reviewed, low-drift. **Dropped as an active target 2026-08-13** — it was originally the fallback goal if the two goals below turned out unreachable; Ling confirmed real data exists for both, so this isn't being actively built on for now. It stays as-is (doesn't hurt anything) and could be revived later. See `rag/README.md` for the generated-artifact policy if it ever gets picked back up.

**The two real goals, per Ling's own framing 2026-08-13: "this business isn't that hard — the only hard parts are maintaining human relationships and pricing."** Both need real data before either is a scopeable engineering task, not a "build a chatbot" problem first.

**1. Relationship/contact mapping.** Motivating example Ling gave: someone asks "who is Liza, what's her role, her connection to TCC" and gets back her account (e.g. Caesars), when the relationship started, who the first contact was, and recent shared projects. Real pain point named: "a lot of the time we face challenge in project is we talk to the wrong person or we didn't find the key person." Data source: ~100GB of Microsoft 365 email archive Ling has access to. Agreed approach — **headers first, not full body parsing**: From/To/Cc/date/subject alone builds a contact-interaction graph (who talked to whom, how often, first-contact date) cheaply and safely; body-content extraction (deeper context, who's the real decision-maker) is a bigger, more sensitive phase 2, likely piloted on one known account (Caesars) before wider rollout. This surfaces real internal PII (client contact names, relationship history) — needs a tighter access model than the passcode-gated admin we accepted for public marketing content. **Blocked on:** how Ling can actually access the 365 archive programmatically (PST export? Graph API? IT-provided dump?) — not yet answered as of 2026-08-13.

**2. Pricing baseline tool.** Not a real-number quoting engine — a **comparables tool** (like real-estate "comps"), deliberately not a regression/ML model given the dataset is small. Data source: ~15 real POs, ~$2M total orders, ranging from area rugs to ballrooms — genuine ground truth Ling already has in hand as PDFs (10-15 pages each, per-area sqm/yard breakdowns, material/wool specs, two quote types — lump sum vs. itemized — plus separate shipping/freight/tax). Given only 15 PDFs, plan is to structure the data by hand into a Supabase table (PO record + line items) rather than build a parsing pipeline for that volume — worth revisiting only if PO volume grows into the hundreds.

Key design points from the discussion, confirmed by Ling:
- **Technique is the primary pricing dimension, not incidental to material/area.** His own words: some techniques are cheap for TCC to execute but read as premium, and that gap — not production cost — is where the real margin and the differentiation from other shops lives. He explicitly called this "hard to put a price on" and confirmed the tool should surface historical technique-level pricing patterns honestly (e.g. "hand-tufted has historically gotten $X-Y/sqft in hotel lobbies"), not try to explain or formulize *why* the premium exists.
- **Freight/shipping/tax must be stripped out of the comparable-price signal**, tracked as separate fields — mixing logistics cost (which varies by destination, not by craft) into "price per sqft by technique" would corrupt the comparison. Lump-sum POs won't yield clean per-area pricing the way itemized ones do — tag records by quote type rather than forcing both into one shape.
- **This is a floor/starting-point tool, not an authoritative quote generator** — Ling's own phrase: "baseline, not real number." A user pulls a historical range for a given technique/venue type, then a human still decides where to land based on relationship, negotiation room, etc. — same judgment as today, just with a documented starting point instead of pure memory. This matters for how confident any future UI is ever allowed to sound.
- **Primary user is Ling himself** — self-described as "a sales engineer who knows a little of everything but isn't an expert, especially not in sales/pricing." The tool is for him first; if it proves useful, the pitch to senior sales (Matt/Marco) is time savings — skip manually reconstructing a baseline from memory, spend their judgment on the premium layer instead — not a way to hand their pricing instincts to junior hires. (Matt/Marco currently deliberately don't teach this to newer staff — worth revisiting the access-model question if/when TCC hires more US sales staff, not blocking now since Ling is the only user in scope.)

**Next concrete step (not yet started):** Ling is sending a real PO sample to nail down the exact schema (fields, how multi-area/multi-technique breakdowns are structured) before building anything — don't guess at the schema in the abstract.
