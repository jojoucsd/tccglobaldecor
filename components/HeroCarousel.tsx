...
export default function HeroCarousel() {
  ...
  const startAutoplay = () => {
    if (prefersReducedMotion) { stopAutoplay(); return; }
  ...
  const next = useCallback(() => goTo((p) => (p + 1) % slides.length), []);
  const prev = useCallback(() => goTo((p) => (p - 1 + slides.length) % slides.length), []);
  ...
  return (
    ...
    >
      <div className="relative w-full overflow-hidden rounded-none md:rounded-2xl bg-black h-[88svh] xs:h-[85svh] sm:h-[85svh] md:h-[85vh] min-h-[440px] md:min-h-[520px]">
        {slides.map((s, i) => {
          const active = i === index;
          if (s.type === 'reveal') {
            return (
              <div key={i} className={`absolute inset-0 transition-opacity duration-700 ease-in-out ${active ? 'opacity-100' : 'opacity-0'}`}>
                <TriptychRevealSlide images={s.images} active={active} prefersReducedMotion={prefersReducedMotion} />
              </div>
            );
          }
        ...
        <div className="pointer-events-none absolute inset-0 z-40 hidden md:flex items-center justify-between p-4">
          <button onClick={prev} className="pointer-events-auto rounded-full bg-white/70 px-3 py-2 text-sm shadow hover:bg-white" aria-label="Previous slide">←</button>
          <button onClick={next} className="pointer-events-auto rounded-full bg-white/70 px-3 py-2 text-sm shadow hover:bg-white" aria-label="Next slide">→</button>
        </div>

        {/* Dots */}
        <div className="absolute bottom-5 left-0 right-0 z-40 flex justify-center gap-2">
          {slides.map((_, i) => (
            <button key={i} onClick={() => goTo(i)} aria-label={`Go to slide ${i + 1}`} className={`h-2 w-2 rounded-full ${i === index ? 'bg-white' : 'bg-white/50'} hover:bg-white`} />
          ))}
        </div>
      </div>
    </section>
